...
class TFNeuralNetBaselineStochasticPolicyOTPSensor:
    def __init__(self, num_input, init_learning_rate=1e-6, min_learning_rate=1e-10, learning_rate_N_max=10000,
        ...
        with tf.name_scope("baseline_network_variables"):
            self._W1_baseline = tf.get_variable("W1_baseline", [num_input, self._layer1_hidden],
                                       initializer=tf.random_normal_initializer(), dtype=dtype)
            self._b1_baseline = tf.get_variable("b1_baseline", [self._layer1_hidden],
                                       initializer=tf.constant_initializer(0), dtype=dtype)
            self._h1_baseline = non_linearity(tf.matmul(self._states, self._W1_baseline) + self._b1_baseline)
            self._W2_baseline = tf.get_variable("W2_baseline", [self._layer1_hidden, self._layer2_hidden],
                                       initializer=tf.random_normal_initializer(stddev=0.1), dtype=dtype)
            self._b2_baseline = tf.get_variable("b2_baseline", [self._layer2_hidden],
                                       initializer=tf.constant_initializer(0), dtype=dtype)
            self._h2_baseline = non_linearity(tf.matmul(self._h1_baseline, self._W2_baseline) + self._b2_baseline)
            self._W_out_baseline = tf.get_variable("W_out_baseline", [self._layer2_hidden, 1],
                                                 initializer=tf.random_normal_initializer(), dtype=dtype)
        ...
        self._baseline_optimizer = tf.train.MomentumOptimizer(learning_rate=value_learning_rate, momentum=0.9, use_nesterov=True)
        self._baseline_loss = tf.losses.mean_squared_error(self._discounted_rewards, self._predicted_baseline)
    ...
    def update_parameters(self, iteration, discounted_return, episode_states):
        ...
        for b in range(len(batches)):
            ...
            states = [row[0] for row in batch]
            rewards = [row[2] for row in batch]
            _, loss = self._sess.run([self._baseline_train_op, self._baseline_loss], feed_dict={
                self._states: states,
                self._discounted_rewards: rewards
            })
