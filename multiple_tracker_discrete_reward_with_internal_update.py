...
def run(args):
    ...
    vel_var = args[5]
    max_num_targets = args[6]
    num_targets = np.random.randint(2,max_num_targets+1)
...
if __name__=="__main__":
    ...
    p = Pool(3)
    experiment_folder_name = "linear_policy_discrete_reward_multiple"
    ...
    job_args = [(method,RBF_components,MLP_neurons,i,experiment_folder_name,vel_var,max_num_targets) for i in range(0,3)]
    p.map(run,job_args)
