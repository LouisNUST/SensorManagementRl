...
window_lag = 10
rbf_var = 1
vel_vars = {0:1E-4,1:5*1E-4,2:1E-3,3:1E-2}
...
def run(args):
    ...
    vel_var = vel_vars[process_index]
    print("Starting Thread:" + str(process_index))

    #Initialize all the parameters
    params ={0:{},1:{},2:{}}
    if method==0:
        params[0]["weight"] = np.random.normal(0, 1, [2, num_states])
    ...
    while episode_counter<N_max:
        ...
        if episode_condition and training:
            ...
            for e in range(0,len(episode_actions)):
                ...
                #if np.isnan(gradiant[0][0]) or np.linalg.norm(params[method]["weight"][0,:])>1E3 \
                 #       or np.linalg.norm(params[method]['weight'][1,:])>1E3:
                  #  condition = False
                   # break

                if np.max(np.abs(gradiant)) > 1E2:
                    condition= False
                    break
...
if __name__=="__main__":
    ...
    p = Pool(4)
    experiment_folder_name = "linear_policy_discrete_reward_final"
    ...
    job_args = [(method,RBF_components,MLP_neurons,i,experiment_folder_name) for i in range(0,4)]
    #run(job_args[0])
    p.map(run,job_args)
